...
where $v_g= \vert d\omega/d\beta \vert_{\beta=\beta_0}$ is the group velocity of the HE$_{11}$ modes at $\omega_0$, and $\Theta \big( b(z-z') \big)$ is a Heaviside function. At $ \br=\br' $, Eq.~\eqref{Eq::ImGreenLocal_general} yields the imaginary part of the dyadic Green's function:
	...
	\end{equation}
where we have used $\omega_0=\omega_{eg}$ to reflect the fact that the quantum transitions between the ground state $ \ket{g} $ and the excited state $ \ket{e} $ of the atom define the response frequency of the light.  The real part of the Green's function at $\br = \br'$ determines the energy level shift of the scatterer. 
