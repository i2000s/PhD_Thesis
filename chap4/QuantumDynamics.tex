...
So far, we have treated light as a classical field. 
In this chapter, we will fully quantize the optical field and study the origin of quantum noise properties of light by redefining the Stokes operators using bosonic operators, and introduce the Hamiltonian for the atom-light interaction for quantum non-demolition (QND) measurement. We will also link the collective spin operators to the traditional squeezing operators and finally introduce the QND-measurement--induced spin squeezing. 

\section[Atom-light coupling with waveguide modes]{Atom-light coupling with waveguide modes}
We consider atoms trapped in a one- or two-sided chain of optical lattice near a nanophotonic waveguide. A probe is launched to perform the quantum measurement using the polarization spectroscopy technique introduced in Chapter~\ref{chap:waveguideinterfaces}. We assume that all atoms are trapped the same distance to the waveguide surface with the same local intensity of the probe. Vacancy defects in the optical lattice are allowed in our theory, and the minimum distance between the atoms is large enough that we can ignore the photon scattering effect among atoms in the dispersive regime. 
...
\begin{subequations}\label{Eq::StokesComponents_general}
	...
	\end{align}
\end{subequations}
They satisfy equal-$z$ commutation relations following from \erf{Eq::InputOutputCommutation_a},
...
\label{eq:chippp}
\end{align}
with the horizontally(H)- and vertically(V)-polarized guided modes, $ \mathbf{u}_p(r'\!_\perp) $, at the atom position $ \br'=(r'\!_\perp,\phi',z') $. 
$ \hat{\mathbf{f}} $ is the total atomic angular momentum operator.
...
The coupling operator, Eq.\eqref{eq:chippp}, includes three terms corresponding to scalar, vector and tensor interactions between the atoms and the probe light which are proportional to $ C_{j'ff'}^{(K)} $ with $ K=0,\,1,\,2 $, respectively.
These terms may vanish depending on the state of the atoms and the polarization state of the probe defined for an atom-light interaction protocol.

Consequently, by decomposing the polarizability tensor into irreducible representations of $\text{SO}(3)$ [Eq.~\eqref{Eq::IrreducibleDecomp}], we can also write the Hamiltonian in the following form~\cite{Deutsch2010a}:
\begin{align}  
...
\end{align} 
Each line above corresponds to the scalar, vector and tensor light shift Hamiltonian. 

\section{Some special states of alkali atoms}
In this section, we highlight two sets of states of alkali atoms, particularly, $ ^{133} $Cs atoms: one is the \emph{clock states}\index{state!clock state}, and the other one is the \emph{stretched states}\index{state!stretched state}. We will use these states and coupled state subspace to design the QND measurement and induced spin squeezing protocols in later chapters. \nd{I am here.}

Beside the states of individual atoms, we will also introduce three collective states in this section. 
...
\subsection{Stretched states and their squeezing subspace}
\begin{figure}[!tbp]
\centering\makebox[\textwidth]{
...
\begin{align}
\ket{J=\frac{N}{2},M_x=J} &=\ket{\uparrow_x}^{\otimes N} = \left(\frac{\ket{\uparrow}+\ket{\downarrow}}{\sqrt{2}}\right)^{\otimes N}\\
&= \frac{1}{2^J}\sum_{M=-J}^J  \sqrt{\left( \begin{smallmatrix}2J\\ J+M \end{smallmatrix} \right)} \ket{J,M}
\end{align}
\end{subequations}
...
&=\left( \begin{matrix} 2J\\ M+J\end{matrix}\right) \left(\frac{1}{2} \right)^{2J}\left(\frac{1}{2} \right)^{M+J} 
\approx \frac{1}{\sqrt{\pi J}} e^{-\frac{M^2}{J}} \quad \text{for } J\rightarrow\infty.
\end{align}
...
\begin{align}
\expect{\Delta J_\perp^2} &= \frac{J}{2}=\frac{N}{4}.
\end{align} 
