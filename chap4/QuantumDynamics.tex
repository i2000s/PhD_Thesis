...
We consider atoms trapped in a one- or two-\nd{``sided" (What do you mean by one- and two-sided chain?)} chain of optical lattice near a nanophotonic waveguide. A probe is launched to perform the quantum measurement using the polarization spectroscopy technique introduced in Chapter~\ref{chap:waveguideinterfaces}. We assume that all atoms are trapped the same distance to the waveguide surface with the same local intensity of the probe. \nd{``Vacancy defects in the optical lattice are allowed"?} Defects of occupations on the optical lattices are allowed in our theory, and the minimum distance between the atoms is large enough that we can ignore the photon scattering effect among atoms in the dispersive regime. 
...
\end{align}
where $\poltens$ is the atomic tensor polarizability operator, given in \erf{Eq::AtomicPolarizabilityF}, for each atom trapped near the waveguide surface at position $\mathbf{r}'_\perp$ in the transverse plane. Here, we ignore any effects of atomic motion and treat the atoms as localized at fixed positions in space. We have also defined $ \hat{S}_i $ as the Stokes vector operators of the light \nd{Omit: indicating its polarization}, which are the quantum counterparts of the classical definition in Eq.~\eqref{eq:S_xybasis}. The vector Stokes operators that describe the polarization of the propagating fields in the quasilinear $HV$-basis are given as below:
...
\label{eq:chippp}
\end{align}
with the horizontally(H)- and vertically(V)-\nd{omit:linearly} polarized guided modes, $ \mathbf{u}_p(r'\!_\perp) $, at the atom position $ \br'=(r'\!_\perp,\phi',z') $. 
...
These terms may vanish depending on the state of the atoms and the polarization state of the probe defined for \nd{a given?} the atom-light interaction protocol.

Consequently, by writing the polarizability tensor in the irreducible representation [Eq.~\eqref{Eq::IrreducibleDecomp}] \nd{``decomposing the polarizability tensor into irreducible representations of $\text{SO}(3)$"}, we can also write the Hamiltonian in the following form~\cite{Deutsch2010a}:
\begin{align}  
