...
&=\hbar\sum_{i,j=0} \chi_{ij}\hat{f}_i\hat{S}_j,\label{eq:heff_chifS}
\end{align}
...
Now suppose that rather than using the magnetic sublevels within a single ground hyperfine state $f$, we begin with a dispersive interface defined on the $m=0$ \emph{clock states}\index{state!clock state} between the $v=3$ and $v=4$ hyperfine states of $ 6S_{1/2} $.  To first order, such states are insensitive to ambient 
...
Since this subspace is a psuedo spin-$ 1/2 $ space, we reduce the total atomic angular moment operator $ \hat{\mathbf{f}} $ to the $ \hat{\mathbf{j}}=\hat{\boldsymbol{\sigma}}/2 $ spin-$ 1/2 $ operator, with $ \hat{\sigma}_z $ defining the spin state transformation relations.
For $ N_A $ atoms, the collective spin operator, 
\begin{align}
\hat{\mathbf{J}} = \sum_n^{N_A} \hat{\mathbf{j}}^{(n)}
\end{align}
defines the collective measurement operators we will see later, where $ \hat{\mathbf{j}}^{(n)} $ is the spin operator for the $ n $th atom. 
...
In the clock state subspace, the light shift Hamiltonian, \erf{eq:heff_uhvalpha}, can then be expressed in these state basis by finding 
the 
matrix elements between two relevant states.  Since the atomic polarizability tensor is block-diagonal grouped by the level energies in 
the ground hyperfine states, we need only to consider coupling between the states within the same 
manifold.  
...
polarizability tensor restricted to the $ (f,m=0) $ clock space. The first block inside of the big bracket is a dyad. Obviously, the result as a trace does not depend on how we rotate the basis inside. 
...
We stick to the choice of quantization axis used above and rewrite Eq.~\eqref{eq:heff_uhvalpha} as
\begin{align}
...
To calculate the Hamiltonian in the clock space, we ignore the spontaneous difference among hyperfine structure sublevels of the excited states and expand
\begin{align}
\chi_{H,\uparrow/\downarrow} & \equiv \chi_{H,f} =- \frac{2\pi \omega_0}{v_g} \bra{f,0} 
	\mathbf{u}^*_H(r^\prime\!_\perp, \phi') \cdot \poltens \cdot 
...
\chi_{V,\uparrow/\downarrow} & \equiv \chi_{V,f} =- \frac{2\pi \omega_0}{v_g} \bra{f,0} 
	\mathbf{u}^*_V(r^\prime\!_\perp, \phi') \cdot \poltens \cdot 
...
\begin{align}
\chi_{H,\uparrow/\downarrow} & \equiv \chi_{H,f} = - \tr \left\{ \mathrm{Im}\left[ 
\GFT^*_{HH}(\br^\prime\!_\perp,\br^\prime\!_\perp) \right] \cdot \tensor{\boldsymbol{\alpha}}^{f0;f0} 
\right\}\label{eq:chiHGalpha}\\
\chi_{V,\uparrow/\downarrow} & \equiv \chi_{V,f} = - \tr \left\{ \mathrm{Im}\left[ 
\GFT^*_{VV}(\br^\prime\!_\perp,\br^\prime\!_\perp) \right] \cdot \tensor{\boldsymbol{\alpha}}^{f0;f0} 
\right\},\label{eq:chiGalpha}
\end{align}
where 
\begin{align} 
\!\!\!\! \GFT_{HH/VV}(\br',\br') &= i2\pi k_0 n_g 
\mathbf{u}_{H/V}(r^\prime\!_\perp,\phi')\mathbf{u}_{H/V}^*(r^\prime\!_\perp,\phi')\\
\tensor{\boldsymbol{\alpha}}^{f0;f0} &\approx \frac{\sigma_0\lambda_{j'}}{4\pi^2}  \sum_q \sum_{f'} \left( 
		\frac{\Gamma}{4 
		\left(\Delta_{ff'}+i\Gamma/2\right) }  \right)   |o^{j'f'}_{jf} |^2 
		|C^{f 0;1q}_{f' q}|^2  \mathbf{e}_q \mathbf{e}_q^*.
\end{align}


Alternatively, we can use the irreducible expansion of the Hamiltonian or the coupling operators [Eq.~\eqref{eq:chippp}] to calculate the coupling strength in the clock space. 
The effective Hamiltonian can be rewritten using the irreducible tensor components as~\cite{Deutsch2010a}
\begin{align}  
...
In the clock state subspace, we can denote the reduced atomic polarizability tensors as 
\begin{align}
\tensor{\alpha}_{\uparrow} &=\bra{f=4,m=0}\poltens\ket{f=4,m=0},\\
\tensor{\alpha}_{\downarrow} &=\bra{f=3,m=0}\poltens\ket{f=3,m=0}.
\end{align}
It can be proven that, in the clock state subspace, the vector component of the atomic polarizability is always zeros due to the symmetry of quantum jumps, which vanishes the crossing terms of the light-shift Hamiltonian involving products between both $ H $ and $ V $ modes vanish as being shown using the approach introduced earlier in this section. 
Using the reduced polarization operators, we rewrite the coupling strengths for the different combination of clock states and polarizations in Eq.~\eqref{eq:Heffupdown} as follows:
\begin{align}
...
\end{align}
in a qubit subspace of the $ f $ hyperfine manifold. These operators only capture a subset of the whole manifold space of the $ f $ hyperfine structure level. Any losses leaked out of the qubit subspace are not tracked. Moreover, they do not provide the information about transfer of coherences~\cite{Norris2014}. Therefore, it's better to extend the subspace to a larger space. 
...
Processing in the same way as for the clock space, the total atom-light interaction Hamiltonian can be projected onto the stretched qutrit subspace and can be simplified to the similar form of Eq.~\eqref{eq:heff_chifS} yet with collective spin operators $ \hat{F}_i $'s. 

Worth noting that, if there is only a transverse linearly polarized electric field at the atom positions, the Hamiltonian may only remain one non-zero term, $ \chi_{33}\hat{F}_z\hat{S}_3 $, which yield a Faraday effect for the light.
If the detuning of the probe is large, we can also ignore the tensor coupling strength related to $ C_{jj'ff'}^{(2)} $ terms in Eq.\eqref{eq:chippp} as the tensor interaction strength ($ \sim 1/\Delta^2 $) is relatively small compared to the vector interaction strength ($ \sim 1/\Delta $)~\cite{Deutsch2010a}. 
The Faraday interaction coupling strength $ \chi_{33} $ may be simplified to only have a vector interaction term, which can be used to design the measurement geometry so that the cooperativity of atom-light interaction reaches its maximum value. We will discuss more in Chapter~\ref{chap:Faraday}.
...
In literature, a Dicke state with $ N $ qubits including $ k $ excitations is also defined as~\cite{Dicke1954,Toth2007Detection} 
\begin{align}
...
\qxd{Maybe a plot for these states on the Bloch sphere.}
...
\end{align}
where $ \Delta J_{out}^2 =\frac{\Delta J_{in}^2}{1+\xi} $, $ \bar{M}(m)=\frac{m}{1+\xi} $, and $ \xi\equiv \frac{\Delta J_{in}^2}{\sigma^2}=\frac{J}{2\sigma^2} $ is the metrological squeezing parameter which characterizes the measurement backaction and the squeezing effect. The ability to resolve the initial quantum variance of $ J_z $ within the resolution of the meter is the key to QND measurement. 
...
\zeta^2 &\equiv \frac{\expect{\hat{F}_\parallel(t=0)}^2}{\expect{\Delta F_\perp^2(t=0)}} \frac{\expect{\Delta F_\perp^2}}{\expect{\hat{F}_\parallel}^2},
\end{align}
...
Now, we go back to the effective Hamiltonian of the spin-photon system described in Eq.~\eqref{eq:Heffupdown}, and rewrite it in terms of the Stokes and spin operators that 
\begin{align}
...
P_{M|m} \propto e^{-\frac{(M-\bar{M}(m))^2}{2\Delta J_{out}^2}},
\end{align}
where the uncertainty of measurement $ \Delta J_{out}^2 =\frac{\Delta J_{in}^2}{1+\xi}=\frac{J}{2(1+\xi)} $ is reduced when $ \xi\equiv \frac{\Delta J_{in}^2}{\sigma^2}=\frac{J}{2\sigma^2}=\frac{\chi_{e\!f\!f}^2}{4}N_A\dot{N}_L \gg 1$. Of course, a larger coupling strength $ \chi_{\rm eff} $ will yield a stronger squeezing effect.
...
The metrological squeezing parameter $ \xi $ can be defined as the ratio between the projection noise and the shot noise. Therefore, we have
\begin{align}
