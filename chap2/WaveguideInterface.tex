...
\begin{align}
\mathbf{E}_x(\br,t) &= \mathcal{E}_{x}\mathbf{e}_x e^{i(k_0z-\omega_0 t)} \text{ and}\\
...
\begin{align}
E_x (\br,t) &= E_0\cos(k_0z-\omega_0 t) \text{ and}\\
...
with properties
\begin{align}
\mathbf{e}_q\cdot \mathbf{e}_{q'}^* &=\delta_{q,q'} \text{ and}\\
...
[Editor's note: This would be a good place to redefine helicity for the reader, unless you're just assuming that the audience is familiar with helicity. The surrounding text, though, indicates that this is not the case since you're defining electric field vectors for them.]
...
\end{align}
where $ \mathcal{E}_\pm $ are complex amplitudes, and $ \mathrm{angle}\left[\mathcal{E}_+\right]$ may not equal to $\mathrm{angle}\left[\mathcal{E}_-\right] $ [Editor's note: I'm confused about what your notation is doing here, are you sure you mean square brackets? I could just be unaware of the notation type you're using, but to me this usually means "floor," not angle].
When $ \mathcal{E}_+ $ and $ \mathcal{E}_- $ have different magnitudes yet the same phase, the light is elliptically polarized with the $ \mathbf{E} $ vector traced as an ellipse defined by the principle axes pointing along $ \mathbf{e}_x $ and $ \mathbf{e}_y $ directions~\cite{Jackson1975}. The semimajor and semiminor axes of the ellipse have a ratio of $ |(1+r)/(1-r)| $, where $ r=\mathcal{E}_+/\mathcal{E}_- $ [Editor's note: I believe you should put this on its own line for clarity]. 
