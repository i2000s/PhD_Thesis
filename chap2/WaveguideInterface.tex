...
\end{align}
\end{subequations}
In Eqs.~\ref{Ertcrla} and~\ref{Ertcrga}, we denote the normalized bound fundamental modes as $ \mathbf{E}^{(\mu)} (\br,t)$ by the superscript $ \mu=(\omega,f,p) $, where $ f=\pm $, as before, denote forward $(+)$ or backward $(-)$ propagation direction, and $ p=\pm$ denote the conterclockwise $(+)$ or clockwise $(-)$ \satomi{Please check that the directions are correct} of polarization. Similar to the quasi-linear case, we use $ \boldsymbol{\mathcal{E}}^{p=\pm} $ to indicate the spatial components of the field with a given circulation pattern. For a linearly polarized \HE mode, the cylindrical components are just the superposition of the two circular fields, 
...
\end{align}
\satomi{can you fit this equation in one line? Or did you purposefully wanted to number them?}
...
\end{align}
Notice that the group index of refraction is not the same as the $ n_{e\!f\!f} $ defined above. We will discuss group velocity and group index of refraction of a waveguide later. See also the notes on \emph{Group velocity of the nanofiber} \satomi{Give reference to the section here}. 
...
\end{align}
Correspondingly, the group velocity, $v_g$, and group index, $n_g$, of refraction for a guided mode in a waveguide can be given by
\begin{align}
v_g &= \left.\dd{\omega}{\beta}\right|_{\beta=\beta_0},\\
n_g &= \frac{c}{v_g}=\left.\dd{\beta}{k}\right|_{\beta=\beta_0}.
\end{align}
Therefore, these quentities of $ HE_{11} $ mode can be calculated from the eigen function of $ \beta $~\cite{LeKien2005a}. 
...
As one simple example, we consider a free-space case where the photon emitter is emerged \satomi{consider 'produced'} in the vacuum background. Then the vector potential $ \mathbf{A}(\br) $ and scalar potential $ \phi(\br) $ are found in the following equations, 
\begin{align}
...
The first column vector of $ \GFT(\br,\br') $, or $ \mathbf{G}_x $, defined in Eq.~\ref{eq:freespaceGFTJ} is formally the electric field due to a point-source current $ \mathcal{J}=\frac{\omega_0}{i}\delta^{(3)}(\br-\br')\mathbf{e}_x $. \satomi{define $\bm{e}_{x}$} Based on Eq.~\ref{eq:freespaceAG0int}, the vector potential from this source current is 
\begin{align}
...
\end{align}
where $ \boldsymbol{\alpha}(\br') $ denotes the polarizability of atoms or dipoles at $ \br' $. We provide the generic field solution under the Born approximation both in volume integration form for continuous medium, and in the discrete summation form for the case with finite number of sources. Note that Born series does not necessarily converge. Usually, verification is required to determine if the perturbation fields are much weaker than the field $ \boldsymbol{\mathcal{E}}_0 $ before using Born series. Depending on the problem, $ \GFT(\br,\br') $ in equations \satomi{put the equation numbers} above can be in any explicit form, which can usually be solved by merely using the boundary condition of the problem without the sources.
...
The $ r\!_\perp $ and $ \phi $ components of the fields can be obtained using Eq.~\eqref{EHzgauss}, and $ \mathcal{B}=\mathcal{H} $ in Gauss units for given $ m $. Also notice, in the nanofiber case we are studying, we can define $ z'=0 $ and $ \phi'=0 $, and hence the factor $ e^{im(\phi-\phi') + i\beta (z-z')} $ in the equations above becomes $ e^{im\phi + i\beta z} $ (also done in refs.~\satomi{cite the papers here}).
...
&\quad + \frac{d_-}{2\sqrt{2}}\left[ i\beta^2J_{m-1}\!\left( pr_\perp \right) +\frac{imp}{r\!_\perp}J_m\!\left( pr_\perp \right)\right] H_{m-1}^{(1)}\!\left( {pr\!_\perp^{\prime} }\right).
\end{align}
More details on deriving the E-field components can be found in \emph{Derivation of free-space E-field components.pdf}\satomi{cite the appendix}. Some properties of Bessel functions are used in deriving these expressions (see appendix)\satomi{cite the appendix}. 
...
The expressions above are formally different from reference\satomi{cite the paper by Klimov}, but are consistent with reference~\cite{Nha1997}. By using some identities of Bessel functions, one should be able to reproduce the expression given in former. We have verified numerically the equivalency of our results. 
...
\end{align}
\end{subequations}
To distinguish the bound and radiation modes contributions, one can use the integral path along $ C_1 $ (see Fig.(\ref{fig:integralpath})) and find the equivalent integral path detouring \satomi{do you mean 'avoiding' or 'through'?} the branch cuts and isolated poles which will be discussed next. 
...
One can obtain the bound and radiation field components with the dipole oriented in $ z $, $ \phi $ and $ r\!_\perp $ directions by substituting the $ \bmc{E}^{(0)}_{m\beta}(r\!_\perp) $ expressions for corresponding cases into Equs.~(\ref{ET0RRes}-\ref{BT0RC2}). 
...
\end{align}
\satomi{Is this above line multiplying the line above? if so, put $\times$.}
...
Numerically, the eigen-wave-vector $ \beta_0 $ is indistinguishable with the intrinsic nanofiber eigen mode wave vector in the regime we are interested in.

Next, we solve for the guided modes by differentiating the functions inside of $ Res $ signs \satomi{in equation what?}, and inserting the values of $ \beta_{1,m=\pm 1} $. %\textcolor{red}{(Q: are those poles all of order 1?)} 
...
Our derivation results were checked by reproducing the decay rates following Klimov's paper~\cite{Klimov2004}. 
...
\end{align}
\satomi{can you not put this above in one line?}
Obviously, the $ x $ axis between $ [-k,k] $ and the entire $ y $ axis are the branch cuts. To separate the branches into upper and lower parts, we can define an arbitrarily small positive number $ \delta\epsilon\rightarrow 0 $ so that Eq.~(\ref{branchcut2}) gives
\begin{align}
xy&=\delta\epsilon.
\end{align}
...
-n_2k<x<n_2k.
\end{align}
In this way, the branch cuts as a hyperbola-type lines are symmetrically separated into top-right and lower-left parts, and are very close to the $ x- $ and $ y- $axes. For the top-right branch, one can choose the integration path as shown in Fig.~\ref{fig:integralpath} in the contour integral detouring \satomi{again, 'avoiding' or 'through'?} the branch cut. 
...
Based on the enhanced QND-measurement--induced spin squeezing protocol we have studied in Ref.~\cite{Qi2017Enhanced}, a key to generate a strong spin squeezing effect (for precise measurement or generating non-classical collective states) is to allow for the mode transformation between the ``local oscilator" mode and the ``signal mode". \satomi{Please fix this sentence:}We can them as the $ H $ mode and $ V $ mode. In general, the field operator for the QND measurement can be written as 
...
We want to avoid this walkoff effect, unless the phase shift between the two modes at atom positions are multiples of $ 2\pi $, which is difficult to implement as we will show next. This follwos discussions held with the collaborators on Ref.~\cite{Qi2017Enhanced}.

\section{Optimal width of a \SWG to tolerate fabrication imperfections}

Four plots shown in Fig.~\ref{fig:ng_rect_dwg} were produced following the discussion with Dr. J. Lee, on determining the
preferable dimension of the waveguides. 
...
In the figure, (a) and (b) show that the group index of refraction $n_g$ reaches the
...
case. Considering that the trapping potential well gets sharper when the
...
nearest side of the waveguide. \satomi{Qi, you probably don't have to say this following sentence.} We haven't considered the quantized
trapping potential-level splitting on the vertical direction, which would be interesting for the experiment.
...
compensate for the phase drafting and extract the pure Faraday rotation
