...
\teves{This would be a good place to redefine helicity for the reader, unless you're just assuming that the audience is familiar with helicity. The surrounding text, though, indicates that this is not the case since you're defining electric field vectors for them.}\qxd{I changed ``definition" to ``concept", which is stated in earlier paragraph. Does it sound ok?}
...
\end{align}
where $ \mathcal{E}_\pm $ are complex amplitudes, and $ \mathrm{angle}\left(\mathcal{E}_+\right)$ may not equal to $\mathrm{angle}\left(\mathcal{E}_-\right) $. \teves{I'm confused about what your notation is doing here, are you sure you mean square brackets? I could just be unaware of the notation type you're using, but to me this usually means "floor," not angle.} \qxd{I changed to round brackets. Hopefully, I don't have this used in other places.}
When $ \mathcal{E}_+ $ and $ \mathcal{E}_- $ have different magnitudes yet the same phase, the light is elliptically polarized with the $ \mathbf{E} $ vector traced as an ellipse defined by the principle axes pointing along $ \mathbf{e}_x $ and $ \mathbf{e}_y $ directions~\cite{Jackson1975}. The semimajor and semiminor axes of the ellipse have a ratio of $ |(1+r)/(1-r)| $, where $ r=\mathcal{E}_+/\mathcal{E}_- $. \teves{I believe you should put this on its own line for clarity.} \qxd{This is a place I am trying to hide the definition, since $ r $ is also a variable for position coordinates, and the $ r $, defined here, will never be used again in other chapters.}
...
&=\frac{2}{\hbar}\left\{ \mathbf{d}\cdot \mathrm{Im}[\GFT_0(\br',\br')+\GFT_R(\br'\br')]\cdot \mathbf{d}^* \right\}.
\end{align}
...
\begin{align}
\mathrm{Im}[\mathbf{G}(\br',\br')] &= \sum_{i=1,2,3} g_i\mathbf{v}_i\mathbf{v}_i,\,\text{with}\, g_i>0,
\end{align}
...
\end{align}
where vector elements $ d'_i=\frac{2}{\hbar}\sqrt{\mathbf{v}_i\cdot \tensor{\boldsymbol{\rho}}\cdot \mathbf{v}_i}$, and $\mathbf{d}'=\sum_i d'_i\mathbf{v}_i$ defines the equivalent dipole orientation of the atomic polarizability transitions. We order $ \Gamma_i $'s by
\begin{align}
