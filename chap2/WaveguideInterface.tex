...
For the \SWG and general geometries, we solve the eigenmodes using a numerical method called \emph{Boundary Element Method}\index{Boundary Element Method} or BEM~\cite{Fallahkhair2008}. This method divides the regions close to the waveguide boundaries into finite grids and solve the wave equations by matching the boundary conditions on the grids. The wave equations are discretized to a set of linear equations. The eigenmodes are found by solving the eigenvalue problem defined by the boundary condition in the frequency domain. It is fairly efficient when we have a fixed frequency point. We can check the accuracy of the BEM calculation with the nanofiber case since we know the analytical solution, which is great in the regime we care about. Then we use BEM to solve the eigenmodes for the \SWG geometry. 
...
For both nanofibers and square waveguides, a relatively simple form for the guided eigenmodes can be expressed in a cylindrical basis $(r_\perp, \phi, z)$ with longitudinal unit vector $\mathbf{e}_z$, oriented along the waveguide axis.  
...
The transverse profile for the quasicircular guided modes, $p = \pm$, is
	\begin{align} \label{Eq::QuasicircularModes_wg}
		\mbf{u}_{b,\pm}(\mathbf{r}_\perp) = \big[\mathbf{e}_{r_{\!\perp}} u_{r_{\!\perp}}(\br_\perp) \pm i \mathbf{e}_\phi u_\phi(\br_\perp) +  i b \mathbf{e}_z  u_z(\br_\perp) \big]e^{ \pm i \phi}, 
	\end{align}
and for the quasilinear guided modes, $p = \{H,V\}$, is
	\begin{subequations} \label{Eq::QuasilinearModes_wg}
	\begin{align}
		\mbf{u}_{b,H}(\mathbf{r}_\perp) = & \sqrt{2} \big[ \mathbf{e}_{r_{\!\perp}} u_{r_{\!\perp}}(\br_\perp) \cos \phi - \mathbf{e}_\phi u_\phi(\br_\perp) \sin \phi +  ib \mathbf{e}_z  u_z(\br_\perp) \cos \phi \big] \\
		\mbf{u}_{b,V}(\mathbf{r}_\perp) = & \sqrt{2} \big[ \mathbf{e}_{r_{\!\perp}} u_{r_{\!\perp}}(\br_\perp) \sin \phi + \mathbf{e}_\phi u_\phi(\br_\perp) \cos \phi +  ib \mathbf{e}_z  u_z(\br_\perp) \sin \phi \big]. 
	\end{align}
	\end{subequations}
The modes are expressed in terms of real-valued functions $ \left[u_{r_{\!\perp}}(\br_\perp), u_\phi(\br_\perp), u_z(\br_\perp)\right] $.
For the HE$_{11}  $ modes of a nanofiber, $u_{r_{\!\perp}}(\br_\perp)=u_{r_{\!\perp}}(r_\perp)$, $u_\phi(\br_\perp)=u_\phi(r_\perp)$, and $u_z(\br_\perp)=u_z(r_\perp)$ depend only on the radial coordinate $r_\perp$ given by Eq.~\eqref{Eq::ProfileFunctions}.

For both waveguides, these eigenmodes usually have non-zero longitudinal component. 
...
\begin{align}
\mbf{u}_{H}(\mathbf{r}'_\perp) = & \sqrt{2} \big[ \mathbf{e}_{r_{\!\perp}} u_{r_{\!\perp}}(\br_\perp') +  i \mathbf{e}_z  u_z(\br_\perp') \big] \\
\mbf{u}_{V}(\mathbf{r}'_\perp) = & \sqrt{2}  \mathbf{e}_\phi u_\phi(\br_\perp') ， 
\end{align}
\end{subequations}
where the $ H $ mode is elliptically polarized, and the $ V $ is linearly polarized. 
Putting the atom on the $ y $ axis will give a similar result by exchanging the $ H $ and $ V $ labels from Eq.~\eqref{eq:uHVrx}.
