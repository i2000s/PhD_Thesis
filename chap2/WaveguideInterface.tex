...
The eigenmodes of a waveguide, in complete cases, should include both the guided (bounded) and radiative (unbounded or unguided) solutions, based on the corresponding waveguide boundary condition problem governed by the Maxwell equations~\cite{Jackson1975}. Below, we will only sketch out the condition to divide the guided and radiative modes using an analogy to the \sch equation, which will be useful to understand the radiation decompositions in sections follow.

\subsubsection{Guided and radiative modes}
...
\end{pmatrix} = 0,
\end{align}
where $ \varepsilon=n^2 $ is the permittivity of the medium and $ k $ is the wavenumber of the modes.
Notice that we are working in the cylindrical coordinate system, and only concentrate on the $ z $-component of the fields since all the other components can be expressed in terms of the $ z $-components of the fields (see detailed derivations in Appendix~\ref{MWE:components}). 
...
The division of guided and radiative modes based on the value of $ \beta $ can also be roughly understood in the perspective of geometric optics as elucidated in Fig.~\ref{fig:Fibermodes}. We regard $ \beta $ as the $ z $-projected wave number of a wave vector of the light ray on $ \vec{k} $ direction. If the ray is incident in an angle that satisfies the total internal reflection condition (in our case, $ k_0<\beta<n_1k_0 $ with $ n_2=1 $), the corresponding modes are guided; otherwise, the modes are unguided with the light leaking out and decaying along $ z $.
...
The solution of the electric field and the fundamental eigenmodes of a cylindrical fiber have been solved in previous works~\cite{Snyder1983Optical,LeKien2014}. We provide a brief review of the method to solve the fiber problem in Appendix~\ref{chap:fibereigenmodes} with solutions of the \HE eigenmodes and its group velocity. The radiative mode solution is less used in this dissertation and can be found in, for example, Ref.~\cite{LeKien2014}. 
...
In this section, we will consider the light response in presence of a photon emitter outside of a dielectric waveguide. 

Formally, the total electric field of the light can be written as 
\begin{align}
\boldsymbol{\mathcal{E}}(\br)=\boldsymbol{\mathcal{E}}_0(\br)+\boldsymbol{\mathcal{E}}_s(\br) ,
\end{align}
where $ \boldsymbol{\mathcal{E}}_0(\br) $ is the bare waveguide field discussed in the last section, and $ \boldsymbol{\mathcal{E}}_s(\br) $ is the scattered field due to the presence of the external photon emitter or the source term.
In general, it's difficult to determine the scattered field $ \boldsymbol{\mathcal{E}}_s(\br) $ with a dielectric boundary condition. That is where the Green's function approach can help us solve the problem by treating the external photon emitter as an integral of unit dipole sources (essentially, as $ \delta $ functions) and solving a simpler unit dipole response function that is the Green's function.

Formally, the total time-dependent electrical field can be rewritten as 
\begin{equation}
...
\end{align}
where we only look at the \emph{positive} signed solution for our example. 
...
\textcolor{red}{To be continued: Retard Green's function for forward and backward solutions, propagator and transverse dyadic Green's function...}
...
\section{Green's function formalism for the atom-waveguide system}
...
\begin{align}
\begin{cases}
0\leq \beta < k, &\rightarrow \emph{unbounded radiative modes;}\\
