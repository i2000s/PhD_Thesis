...
Based on our simulations, we find that if $ r^\prime\!_\perp\ge 1.8a $ for the nanofiber or $ r^\prime\!_\perp\ge 1.2w $ for the \SWG, the total decay rates are barely modified from the natural linewidth, despite of various dipole orientations.
That is we can safely ignore the modification of the decay rates due to the presence of the two types of waveguides when the atoms are trapped in a far distance from the waveguide surface. I have to point out that, how far is far enough to valid our statement above also depends on the choice of quantization axis. 
For the \SWG case, the azimuthal angle of the trapping position will also affect the estimation of decay rate modification, but we will only consider the particular azimuthal trapping angle discussed above, which could be the easiest to implement in experiments.
Based on our simulations with other choices of quantization axis, $ r\!_\perp>1.8a $ for the nanofiber or $ r\!_\perp>1.3w $ seem to be a good range when the waveguide modification effects are negligible. We will work in these ranges for most of this dissertation work to explore the quantum measurement dynamics.

% MakePlot script and data files available in the FaradayProtocol repo.
\begin{figure}[!tbp]
  \centering
...
A side note: the atomic polarizability tensor is positive semidefinite, which indicates the controllability of the photon modes via controlling the atomic state. Our formalism enables us to treat this system as a linear system for controlling light with atoms or vice verse. This geometric perspective might be helpful to design quantum tomography and control protocols based on the nanophotonic waveguides and atoms quantum interface.
