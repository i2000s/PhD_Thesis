...
Therefore, the optical transition rates from $ \ket{F_a,m=0} $ to $ \ket{F_b,m=0} $ can be given by
\begin{align}
\gamma_{F_a\rightarrow F_b} &= \sum_{F_{a'},F_{b'},q} \left| \bra{F_b} \hat{W}_q^{F_{b'}F_{a'}}\ket{F_a}\right|^2\\
&= \sum_q \frac{\Gamma\Omega^2(\mathbf{r}')}{4}\sum_{F'} \left|  \frac{o^{J'F'}_{JF_b}\! o^{J'F'}_{JF_a}\!  C^{F_b 0;1q}_{F' q}\! C^{F_a 0;1q}_{F' q}\! \left(\mathbf{e}^*_{q}\! \cdot\! \mathbf{e}_L \right)}{ \Delta_{F'F_a} \!\!+\! i\Gamma/2} \right|^2.
\end{align} 

Using the results above, one can write the stochastic master equation for the continuous measurement using the probe light discussed in this section in the following form...


...

The measurement operator can be given by
\begin{align}
\hat{M} &= \sum_i^{N_A} \int_0^T \mathrm{d}t' \hat{X}_{\bar{D}}^{\rm out} (t'-(z_{D}-z_i')/v_p)\\
&= N_A \int_0^T \mathrm{d}t' \hat{X}_{\bar{D}}^{\rm out} (t'),
\end{align}
where $ T $ and $ Z_D $ are the integration time and position of the detector, $ z_i' $ is the $ z $-coordinate of the $ i $-th atom. 
In the last step above, we have assumed the transition time of the light passing through the ensemble of atoms is much shorter than the atomic dynamics and the sampling time of the detector. 

Using the input-output formalism result of photon annihilation operators that 
