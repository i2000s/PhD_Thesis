...
\begin{figure}
\centering\makebox[\textwidth]{\includegraphics{./Figs/contourplot_transf}
%\begin{tikzpicture}[scale=0.99,cap=round]
%% Configurable parameters
%\def\gap{0.25}
%\def\bigradius{3}
%\def\littleradius{0.2}
%
%% Styles
%\tikzstyle{axes}=[arrows={-latex}]
%%\tikzstyle{integralpath}={decorate, decoration={ markings,  % This schema allows for fine-tuning the arrows.
%%      mark=at position 0.33 with {\arrow{latex}}, 
%%      mark=at position 0.66 with {\arrow{latex}}}}
%\tikzstyle{important line}=[thick]
%\tikzstyle{branch cut}=[densely dashed]
%%\tikzstyle{information text}=[rounded corners,fill=red!10,inner sep=1ex]
%\tikzstyle{dot}=[circle,inner sep=1pt,fill,label={#1},name=#1]
%
%% Coordinates and points.
%\coordinate (O) at (0,0);	% Origin.
%\coordinate (A) at ({-sqrt(\bigradius^2-\gap^2)},\gap);	% Left-bottom end of the integral path.
%\coordinate (B) at ({sqrt(\bigradius^2-\gap^2)},-\gap);	% Right-bottom end of the integral path.
%\coordinate (C) at (\gap,{sqrt(\bigradius^2-\gap^2)});	% Top right point on the circle.
%\coordinate (D) at (-\gap,{sqrt(\bigradius^2-\gap^2)});	% Top left point on the circle.
%\coordinate (E) at (\bigradius/3,0); 	% The k point.
%\coordinate (F) at (\bigradius*0.55,0); 	% The pole at the positive side of the real axis.
%\coordinate (G) at (\bigradius*2.4/3,0);	% The k\sqrt{\epsilon} point.
%\coordinate (H) at (-\bigradius/3,0);	% The -k point.
%\coordinate (I) at (-\bigradius*2.4/3,0); 	% The -k\sqrtt{\epsilon} point. 
%
%% The graphic
% % help grid
%% \draw[style=help lines,step=0.5cm] (-2.0,-2.0) grid (2.0,2.0);
%  
%% Draw axes.
%\draw [axes] (-1.25*\bigradius, 0) -- (1.25*\bigradius,0) node [above] {$Re(\beta)$};
%\draw [axes] (0, -1.2*\bigradius) -- (0, 1.25*\bigradius) node [right] {$Im(\beta)$};
%\draw (O) node [below left,xshift=0.99*\bigradius,yshift=0.5*\bigradius] {O};
%
%% Draw Integral path C1 along the real axis. 
%\draw [blue, decoration={ markings,  % This schema allows for fine-tuning the arrows.
%      mark=at position 0.33 with {\arrow{latex'}}, 
%      mark=at position 0.66 with {\arrow{latex'}}},postaction={decorate}] (A) -- (-1.414*\gap,\gap) to [out=0,in=135] (O)  
%to [out=-45,in=180] (1.414*\gap,-\gap) -- (B);
%
%% Draw the half circle path with correct ends.
%\begin{scope} [even odd rule]
%\clip (-\bigradius*1.1,-\gap) rectangle (\bigradius*1.1,\bigradius*1.1) 
%{(-\bigradius*1.1,-\gap) -- (-\bigradius*1.1,\gap) -- (-\gap,\gap) -- (-\gap,\bigradius*1.1) -- (\gap,\bigradius*1.1) -- (\gap,-\gap) -- cycle};
%\draw[red, decoration={ markings,  % This schema allows for fine-tuning the arrows.
%      mark=at position 0.125 with {\arrow{latex'}}, 
%      mark=at position 0.375 with {\arrow{latex'}}},postaction={decorate}] (O) circle (\bigradius);
%\end{scope}
%
%% Draw branch cuts.
%\draw[branch cut] (0.1*\gap,1.19*\bigradius) .. controls (0.2*\gap,\bigradius/15).. (E) node [above, yshift=6] {$k$};
%\draw (E) node[dot] {};
%\draw[branch cut] (-0.1*\gap,-1.19*\bigradius) .. controls (-0.2*\gap,-\bigradius/15).. (H) node [below] {$-k$};
%\draw (H) node[dot] {};
%
%% Draw branch cut contour.
%\draw[green,decoration={ markings, % This schema allows for fine-tuning the arrows.
%      mark=at position 0.2 with {\arrow{latex'}}, 
%      mark=at position 0.4 with {\arrow{latex'}},mark=at position 0.62 with {\arrow{latex'}},mark=at position 0.81 with {\arrow{latex'}}},postaction={decorate}] 
%      plot [smooth, tension=0.7] coordinates {(C) (1.01*\gap,\bigradius/2) (1.6*\gap,\bigradius/6) (0.33*\bigradius,\bigradius/14) (0.39*\bigradius,-0.01*\bigradius) (0.3*\bigradius,-\bigradius/20) (0.03*\bigradius,0.03*\bigradius) (-0.9*\gap,\bigradius/3) (D)};
%      %(C) ..controls (0.1*\gap,-\bigradius/8) and (\bigradius*0.3,\bigradius/8).. (0.39*\bigradius,0);
%%\draw[green,decoration={ markings, % This schema allows for fine-tuning the arrows.
%%      mark=at position 0.25 with {\arrow{latex'}}, 
%%      mark=at position 0.75 with {\arrow{latex'}}},postaction={decorate}] (0.39*\bigradius,0) ..controls (\bigradius*0.33,-0.8*\gap) and (-\gap*2,-\bigradius/5).. (D);
%
%% Draw the contour for the pole.
%\draw[cyan, decoration={ markings,  % This schema allows for fine-tuning the arrows.
%      mark=at position 0.2 with {\arrow[sloped,rotate=-15,scale=0.9]{latex' reversed}}, 
%      mark=at position 0.7 with {\arrow[sloped,rotate=-15,scale=0.9]{latex' reversed}}},postaction={decorate}] (F) circle (\bigradius/18);
%      %arc (360:0:\bigradius/18);
%\draw(F) node[dot] {};
%\draw (F) node[above,yshift=5]{$\beta_0$}; 
%
%% Draw other nodes and labels.
%\draw (G) node[dot] {};
%\draw (G) node[above] {$k\sqrt{\varepsilon}$};
%\draw (I) node[dot] {};
%\draw (I) node[below] {$-k\sqrt{\varepsilon}$};
%\draw (A) node[left] {$ -\infty $};
%\draw (B) node[right] {$ \infty $};
%\draw[->] (-0.8,1.5) node[above,xshift=-15] {Branch cut} -- (0.11,0.65);
%\draw[->] (2.7,-0.95) node[below,xshift=5] {Pole} -- (0.56*\bigradius,-0.02*\bigradius);
%\draw[blue] (-1.5,1.2*\gap) node[above] {$C_1$};
%\draw[->] (0.5,-0.95) node[below,green] {$ -C_2 $} -- (0.5,-0.14);
%%\draw[->] (0.75,-0.95) -- (0.5*\bigradius,-0.14);
%
%
%\end{tikzpicture} 
}
...
\end{align}
\end{subequations}
To distinguish the bound and radiation modes contributions, one can use branch cuts and isolated poles to separate the contour integral along $ C_1 $ (see Fig.(\ref{fig:integralpath})). The bound modes are associated with poles, and hence can be represented as residues as below. 
...
The radiation modes are associated with the branch cuts $ C_2 $ which is presented in Ref.~\cite{Klimov2004} and in Fig.(\ref{fig:integralpath}).
...
Comparing Equ.~\eqref{eq:Gi} with Equ.~\eqref{eq:Maxwellwithsource2}, we find that Equ.~\eqref{eq:Gi} is exactly the chromatic wave equation of $ \mathbf{E}(\br) $ when there is a unitary dipole source orientated along $ \mathbf{e}_i $ direction and placed at $ \br' $. That is to say that once we solve the electric field components with unitary dipole source orientated along all $ \mathbf{e}_i $ directions, the columns of the dyadic function is just the corresponding field components. Concretely, in the cylindrical coordinate system, the dyadic Green function elements correspond to the following electric field components emitted by a unitary dipole source orientated in the three orthogonal basis:
\begin{equation}
...
The \textit{approximate} approach:
...
&\approx -\frac{L}{2\pi}\sum_{m,f=\pm 1}\int \mathrm{d}\beta \frac{c^2\mathbf{u}_{m}(r\!_\perp)\mathbf{u}^*_{m}(r_{\!\perp}^\prime)}{2\omega_0v_g(\beta-\beta_0)}e^{if\beta(z-z')+im(\phi-\phi')}\\
&= -\frac{L}{2\pi} \! \sum_{m,f=\pm 1}\! 2\pi i\, \mathrm{Res}\left[ \frac{c^2\mathbf{u}_{m}(r\!_\perp)\mathbf{u}^*_{m}(r_{\!\perp}^\prime)}{2\omega_0v_g(\beta-\beta_0)}e^{if\beta(z-z')+im(\phi-\phi')}\right]\\
&= -\frac{ic^2L}{2\omega_0v_g} \sum_{m,f=\pm 1} \mathbf{u}_{m}(r\!_\perp)\mathbf{u}^*_{m}(r_{\!\perp}^\prime)e^{if\beta_0(z-z')+im(\phi-\phi')},
\end{align}
where $ \beta_0 $ is the propagation constant with a given $ \omega_0 $ for the input/emission field. By using the relation that 
\begin{align}
k_0 &=\frac{\omega_0}{c}\\
n_{e\!f\!f}&=\sqrt{\varepsilon_{e\!f\!f}}= \frac{c}{v_g}
\end{align}
the dyadic Green function for the guided modes can also be written as
\begin{align}
\mathbf{G}^g(\br,\br') &\approx -\frac{in_{e\!f\!f}L}{2k_0} \sum_{m,f=\pm 1} \mathbf{u}_{m}(r\!_\perp)\mathbf{u}^*_{m}(r_{\!\perp}^\prime)e^{if\beta_0(z-z')+im(\phi-\phi')}.
\end{align}
...
\textcolor{red}{The problem is that the expression of the dyadic Green function of the guided modes fit from the normal numerical approach should be 
\begin{align}
\mathbf{G}^g(\br,\br') &\approx -\frac{i}{2n_{e\!f\!f}\beta_0} \sum_{m,f=\pm 1} \mathbf{u}_{m}(r\!_\perp)\mathbf{u}^*_{m}(r_{\!\perp}^\prime)e^{if\beta_0(z-z')+im(\phi-\phi')}.
\end{align}
It means that we should make $ L=k_0/\varepsilon_{e\!f\!f}\beta_0 $ which does not have a clear physics meaning. I will check the numerical calculation shortly. 
}

\textbf{Calculation of $ v_g $ and $ n_{e\!f\!f} $}:

By its definition, the group velocity for a guided mode in a waveguide is given by
\begin{align}
v_g &= \frac{P}{W},
\end{align}
where the total power $ P $ and the energy per unit length stored in the waveguide are defined respectively as
\begin{align}
...
\end{align}
Now that, the group velocity can be retrieved through
\begin{align}
v_g&=\frac{c}{n_{e\!f\!f}}.
\end{align}

Also, to normalize the guided mode functions, we can define the normalization factor to be~\cite{LeKien2005}
\begin{align}
...
\end{align}
where the factor $ A $ is defined in Equ.~\eqref{eq:A} related to the input power. 
...
\begin{align}
C_{mf} &= i2\pi k_0 n_{e\!f\!f}\alpha \!\int\! \mathrm{d}^2r\!_\perp n^2_{e\!f\!f} {\mathbf{u}^f_m}^*(\mathbf{r}\!_\perp,z)\cdot\!\!\!\!\!\! \sum_{f',m'=\pm 1}\!\!\!\! \left[\mathbf{u}_{m'}^{f'}(\mathbf{r}\!_\perp,z){\mathbf{u}^{f'}_{m'}}^*(\mathbf{r}^\prime\!_\perp,z') \right]\!\cdot\! \mathbf{u}_1^1(\mathbf{r}^\prime\!_\perp,z')\\
&= i2\pi k_0 n_{e\!f\!f}\alpha \sum_{f'=\pm 1} \mathbf{u}^*_m(\br'_{\!\perp})\cdot \mathbf{u}_1(\br'_{\!\perp})e^{i\beta_0 (f'-f)(z-z')}.
\end{align}
...
\begin{align}
C_{mf} &\approx C_m\equiv  i2\pi k_0 n_{e\!f\!f}\alpha  \mathbf{u}^*_m(\br'_{\!\perp})\cdot \mathbf{u}_1(\br'_{\!\perp})\\
&= i2\pi k_0 n_{e\!f\!f}\alpha  \mathbf{u}^*_m(r'_{\!\perp})\cdot \mathbf{u}_1(r'_{\!\perp})e^{i(1-m)\phi'}.
\end{align}
...
&=\left[1 +i2\pi k_0 n_{e\!f\!f}\alpha  | \mathbf{u}_1(r'_{\!\perp})|^2 \right] E_0n_{e\!f\!f}\mathbf{u}_{1}^{1} (\br)\\
&= \exp\left[i2\pi k_0 n_{e\!f\!f}\alpha  | \mathbf{u}_1(r'_{\!\perp})|^2 \right] E_0n_{e\!f\!f}\mathbf{u}_{1}^{1} (\br)\\
&= te^{i\delta\phi}E_0n_{e\!f\!f}\mathbf{u}_{1}^{1} (\br), 
\end{align}
where the factor change of the mode's amplitude is 
\begin{align}
t=\exp\left[\Im[C_{11}]\right]=\exp\left[ -2\pi k_0 n_{e\!f\!f}\Im[\alpha]  | \mathbf{u}_1(r'_{\!\perp})|^2 \right]
\end{align}
and the phase shift is
\begin{align}
\delta\phi &= \Re[C_{11}]\\
&=2\pi k_0 n_{e\!f\!f}\Re[\alpha]  | \mathbf{u}_1(r'_{\!\perp})|^2\\
&= 2\pi \Re[\alpha] \frac{\omega_0}{v_g}  | \mathbf{u}_1(r'_{\!\perp})|^2\\
&= 2\pi \Re[\alpha] \frac{\omega_0}{v_gA'_{e\!f\!f}},
\end{align}
...
\begin{align}
\mathbf{E}_{-}(\br) &=  i2\pi k_0 n_{e\!f\!f}\alpha  \mathbf{u}^*_{-1}(r'_{\!\perp})\cdot \mathbf{u}_1(r'_{\!\perp})e^{i2\phi'}E_0n_{e\!f\!f}\mathbf{u}_{-1}^1(\br).
\end{align}
