...
\begin{align}
\gamma_{\uparrow\uparrow} = \gamma_{\downarrow\downarrow}=\frac{\dot{N}_L}{2} \left( \chi_{H,\uparrow}+\chi_{V,\uparrow}\right)|_{\omega=\omega_{\rm magic}} \equiv \gamma_m, 
\end{align}
and hence
\begin{align}
\hat{H}_{\rm eff} &= -\hbar \gamma_m \left( \op{\uparrow}{\uparrow}+\op{\downarrow}{\downarrow} \right).
\end{align}
\textcolor{red}{Note: this may not be true since the magic wavelength condition is only satisfied for the coherent part of the Hamiltonian.}
...
\end{align}
For example, the loss part of the effective Hamiltonian at a magic frequency can be given by
\begin{align}
\hat{H}_{\rm loss} &= -i\hbar \Im\left[ \gamma_m\right] \left( \op{\uparrow}{\uparrow}+\op{\downarrow}{\downarrow} \right)\\
&=  i\dot{N}_L\frac{\mathbin{\color{red}{\pi}} \hbar \omega_0}{v_g} \!\!\sum_{F'}\! \sum_q \Im\left[ \alpha_0\left( 4,\!F'  \right)\right]  \left[ |\mathbf{e}_q \cdot 
\mathbf{u}_{H}^*(r^\prime\!\!_\perp,\phi')|^2 + |\mathbf{e}_q \cdot 
\mathbf{u}_{V}^*(r^\prime\!\!_\perp,\phi')|^2\right] \nonumber\\
&\quad\quad\quad\quad\quad\quad\quad\quad\quad |o^{J'F'}_{J4} |^2 
	|C^{4 0;1q}_{F' q}|^2 \left( \op{\uparrow}{\uparrow}+\op{\downarrow}{\downarrow} \right).
\end{align}
\textcolor{red}{Note: again, this may not be true and the imaginary parts of $ \gamma_{\uparrow\uparrow} $ and $ \gamma_{\downarrow\downarrow} $ may not be equal if the magic wavelength condition is only satisfied for the coherent part of the Hamiltonian.}
...
\end{align}
with local Rabi frequency $\Omega(\mathbf{r}') = 2\bra{P_{J'}}| d | \ket{S_{1/2}} \mathcal{E}^{(+)}_L(\mathbf{r}')/\hbar$ and the positive-frequency amplitude treated as c-numbers by
\begin{align}
...
Now we generalize one-atom system to an $ N_A $-atom system by replacing $\hat{W}_q^{F}\rightarrow \hat{W}_q^{F}(\br_i)  $.
...
The master equation describing expectation values of an arbitrary operator $ \hat{O} $ evolve as
%\begin{align}
...
%\end{align}
\begin{align}
\dt{\expect{\hat{O}}} &= -\frac{i}{\hbar}\expect{\left[\hat{H}_{\rm loss},\hat{O} \right]}+ \expect{\!\sum_{F,q,i}\! \hat{W}_q^{F}(\br_i) \hat{O} \hat{W}_q^{F \dagger}(\br_i) }\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{O}} + \expect{\!\!\sum_{F,q,i}\!\! \gamma_{F,q} \op{F,0}{F,0}^{(i)} \hat{O} \op{F,0}{F,0}^{(i)} },
\end{align}
...
For instance, 
\begin{align}
\dt{\expect{\hat{J}_z}} &= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z} \!+\! \expect{\! \sum_{F,q,i} \!\! \gamma_{F,q} \op{F,0}{F,0}^{(i)} \hat{J}_z \op{F,0}{F,0}^{(i)} }\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z}  + \sum_i^{N_A} \frac{1}{2}\sum_q \expect{ \gamma_{4,q} \op{\uparrow}{\uparrow}^{(i)} -  \gamma_{3,q}\op{\downarrow}{\downarrow}^{(i)}  }\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z} + \left[\sum_q(\gamma_{4,q}-\gamma_{3,q})  \right]\frac{N_A}{2}\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z} +\gamma_-\frac{N_A}{2},\label{eq:expJzeq}
\end{align}
...
In the meantime, 
\begin{align}
\dt{\expect{\hat{J}_z^2}} &=  -2\Im\left[\gamma_m \right]\expect{\hat{J}_z^2} \!+\! \expect{\! \sum_{F,q,i} \!\! \gamma_{F,q} \op{F,0}{F,0}^{(i)} \hat{J}_z^2 \op{F,0}{F,0}^{(i)} }\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z^2} \!+\! \left[\sum_q(\gamma_{4,q}+\gamma_{3,q})  \right]\frac{N_A}{4}\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z^2} \!+\! \gamma_+\frac{N_A}{4},\label{eq:expJz2eq}
\end{align}
with $ \gamma_+ = \sum_q(\gamma_{4,q}+\gamma_{3,q}) $.

Similarly, 
\begin{align}
\dt{\expect{\hat{J}_x}} &= -2\Im\left[\gamma_m \right]\expect{\hat{J}_x} \!+\! \expect{\! \sum_{F,q,i} \!\! \gamma_{F,q} \op{F,0}{F,0}^{(i)} \hat{J}_x \op{F,0}{F,0}^{(i)} }\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_x} ,\\
\dt{\expect{\hat{J}_x^2}} &= -2\Im\left[\gamma_m \right]\expect{\hat{J}_x^2} \!+\! \expect{\! \sum_{F,q,i} \!\! \gamma_{F,q} \op{F,0}{F,0}^{(i)} \hat{J}_x^2 \op{F,0}{F,0}^{(i)} }\\
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_x^2} \!+\! \gamma_+\frac{N_A}{4}.
\end{align}

The evolution of the atom population operator on the clock-state subspace, $ \hat{N}_A=\sum_i^{N_A}\hat{I}^{(i)} $, evolves as
\begin{align}
\dt{\expect{\hat{N}_A}} &= -2\Im\left[\gamma_m \right]\expect{\hat{N}_A} + \expect{\!\!\sum_{F,q,i}\!\! \gamma_{F,q} \op{F,0}{F,0}^{(i)} \hat{N}_A \op{F,0}{F,0}^{(i)} }\\
&=-2\Im\left[\gamma_m \right]\expect{\hat{N}_A} + \gamma_+N_A.
\end{align}
...
Similarly, one can solve the equations for $ \expect{\hat{J}_x} $ and $ \expect{\hat{N}_A} $ to give
\begin{align}
\expect{\hat{J}_x(t)} &= \expect{\hat{J}_x(0)}e^{-2\gamma_st} = \frac{N_A}{2} e^{-2\gamma_st}\\
\expect{\hat{N}_A(t)} &= \frac{N_A}{2}\left[ \frac{\gamma_+}{\gamma_s} + \left( 2- \frac{\gamma_+}{\gamma_s}\right)e^{-2\gamma_st}\right].
\end{align}
...
\textcolor{red}{To be included: the measurement effect in generating spin squeezing or the $ \Delta J_z^2 $ term. A quick fix is below:}
Equs.~\eqref{eq:expJz2solscs} and~\eqref{eq:expJzsolscs} only describe the optical pumping dynamics of the spin system. The measurement effect will yield a $ -\kappa \left(\Delta J_z^2 \right)^2 $ term in the differential equation of $ \Delta J_z^2 $. So, we can use the relationship that
\begin{align}
...
&= -2\Im\left[\gamma_m \right]\expect{\hat{J}_z^2} \!+\! \gamma_+\frac{N_A}{4} 
 - 2\expect{\hat{J}_z} \left[ -2\Im\left[\gamma_m \right]\expect{\hat{J}_z} +\gamma_-\frac{N_A}{2}\right]\\
&= -2\gamma_s \Delta J_z^2 + \gamma_+\frac{N_A}{4}+4\gamma_s\expect{\hat{J}_z}^2 -N_A\gamma_-\expect{\hat{J}_z}\\
&=  -2\gamma_s \Delta J_z^2 + \gamma_+\frac{N_A}{4},
\end{align}
...
\begin{align}
\dt{\Delta J_z^2} &= -\kappa \left(\Delta J_z^2 \right)^2 -2\gamma_s \Delta J_z^2 + \gamma_+\frac{N_A}{4}.
\end{align}
...
\begin{align}
A = \kappa, &\quad  B = 2\gamma_s, &\quad C=\gamma_+\frac{N_A}{4} \quad\text{and}\quad &\Delta J_z^2(0)=N_A/4
\end{align}
to obtain the solution for $ \Delta J_z^2(t) $ as
\begin{align}
\Delta J_z^2(t) &= \frac{ \sqrt{4\gamma_s^2 + \kappa \gamma_+N_A}\frac{N_A}{4} - (\gamma_s -\gamma_+)\frac{N_A}{2} \tanh \left( \sqrt{4\gamma_s^2 +  \kappa \gamma_+N_A}  \frac{t}{2} \right) }{\sqrt{4\gamma_s^2 + \kappa \gamma_+N_A} + (\kappa N_A/2 + 2\gamma_s) \tanh \left( \sqrt{4\gamma_s^2 + \kappa \gamma_+N_A}  \frac{t}{2} \right) }.
\end{align}
\textcolor{red}{Note: two-body operators are not considered in this calculation.}
...
\begin{figure}
\centering\makebox[\textwidth]{
\includegraphics[width=0.65\textwidth]{./Figs/xi_magic44}}
\caption{Spin squeezing evolution as a function of time. Parameters used: $ \Omega/2\pi=52 $ MHz, $ N_A=100 $. }
\end{figure}
