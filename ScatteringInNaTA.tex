...
\newcommand{\fidx}{\eta}
\newcommand{\unittens}{\tensor{\mathbbm{1}}}
...
\end{align}
and can be solved by solving the corresponding dyadic Green functions\index{Green function!dyadic Green function} in the frequency domain\footnote{If we define the Green's function by $\left[ -\nabla\times\nabla\times + n^2\frac{\omega^2}{c^2} \right] \mathbf{G}(\br,\br') = \mathbbm{1}\delta(\br-\br')$, then the source term of the corresponding field will be $S(\br)=-4\pi \frac{\omega^2}{c^2} \boldsymbol{\chi}(\br)$; there will be a $4\pi\omega^2/c^2$ factor over other quantities as well. }
\begin{align}
\left[ -\nabla\times\nabla\times + n^2\frac{\omega^2}{c^2} \right] \mathbf{G}(\br,\br') &= -4\pi \frac{\omega^2}{c^2}\mathbbm{1}\delta(\br-\br'). \label{eq:dyadicGF}
\end{align}
...
\begin{align}
\sum_{f,p}\int \mathrm{d}\mathbf{k} \,n_r^2\mathbf{u}^{(\mu)}(r\!_\perp )\mathbf{u}^{(\mu)*}(r'\!_\perp ) &= \mathbbm{1}\delta^T(r\!_\perp-r'\!_\perp),\\
\sum_{m,p}\int \mathrm{d}\mathbf{k} \, n_r^2\mathbf{u}^{(\nu)}(r\!_\perp )\mathbf{u}^{(\nu)*}(r'\!_\perp ) &=\mathbbm{1}\delta^T(r\!_\perp-r'\!_\perp).
\end{align}

As shown in the earlier sections, in general, the output field in presence of a single atom can be written as
\begin{align}
\mathbf{E}(\br) 
&=\mathbf{E}_0(\br)+\alpha \mathbf{G}(\br,\br')\cdot \mathbf{E}_0(\br')
\end{align}
...
\begin{align}
\left[ -\nabla\times\nabla\times + n^2\frac{\omega^2_0}{c^2} \right] \mathbf{G}(\br,\br') &= -4\pi k_0^2\mathbbm{1}\delta(\br-\br'),
\end{align}
...
\begin{align}
\left[ -\nabla\times\nabla\times + n^2k_0^2 \right] \mathbf{G}(\br,\br') &= -4\pi k_0^2\mathbbm{1}\delta^{(3)}(\br-\br'),
\end{align}
