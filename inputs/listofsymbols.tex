...
\symb{$C^{(r)}_{j'ff'}$, $C^{(r)}_{j'f}$}{Rank-$r$ hyperfine structure atomic transition coefficients connecting ground states with fine structure quantum number $ j $ and hyperfine structure $ f $ and the excited state counterpart denoted with prime ($ \prime $) }\\
