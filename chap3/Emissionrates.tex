...
If we set the dipole moment $ \mathbf{d}=\bra{g}\hat{\mathbf{d}}\ket{e} $ oriented along the $ \mathbf{e}_n $ direction, and define the partial LDOS due to the dipole  by~\cite{Moroz1995Density,Novotny2012}\footnote{Also see Appendix~\ref{chap:freespacegreenfunction} for the total photonic LDOS.}
\begin{align}
...
Without the influence of a nearby material boundary, free-dipole-induced $ \Gamma_0 $ is polarization state independent. 
...
This equivalent dipole comes from the ``no-preference" of the free decay paths in the case that the atom is in a completely mixed excited state. But we may not be able to write down one dipole moment vector to represent mixed states of atoms. We will discuss this more in the next section (Sec.~\ref{sec:waveguideinterfacegeometry}).
...
The rate of emission $\Gamma_rad$ corresponds to the rate of ``diffuse scattering," which leads to decoherence in the atom light interface. 
...
The line code of this figure follows Fig.~\ref{fig:decayrates_hyperfine_D2_qyqz}. %Note that, $ \ket{f'=5,m'=4} $ to $ \ket{f=3,m=3} $ is forbidden based on the selection rules. 
Note that both $ \ket{f'=5,m'=4} $ to $ \ket{f=4,m=3} $ and $ \ket{f'=5,m'=4} $ to $ \ket{f=4,m=4} $ decays contribute to the spontaneous emission rate of $ \ket{f'=5,m'=4} $ sublevel. } \label{fig:D2line_decaylines}
\end{figure}
...
&= \frac{1}{\hbar} \bra{g}\hat{\mathbf{d}}\ket{e}\cdot \mathrm{Re}\left[ \tensor{\mathbf{G}}_{ind,rad}(\br',\br';\omega_{eg})+\tensor{\mathbf{G}}_{gyd}(\br',\br';\omega_{eg})\right]\cdot \bra{e}\hat{\mathbf{d}}\ket{g}.
\end{align}
...
Based on a naive intuition that the confinement of light might be stronger for a waveguide with a square cross-section than a round one, a \SWG may have some advantages of enhancing the quantum measurement of some type over the nanofiber geometry. 
