...
In this dissertation, we will study a cylindrical nanofiber geometry and a square waveguide geometry towards more general integrated waveguides, which can trap atoms with a similar two-color scheme as the nanofiber case. 
...
\begin{figure}[!tpb] % Poincare sphere.
   ...
   The red shadows indicate the possibility distributions on the Bloch sphere defined by the collective spin $ \hat{\mathbf{J}} $ vector operator.
   }
...
When we plot a squeezed collective state on a Bloch sphere that represents the possibility distribution of the state, the uncertainty region is squeezed from the initial spin coherent state (see Fig.~\ref{fig:collectivestatesonblochsphere}).  
