...
This dissertation focuses on solving these problems in the context of quantum measurement and spin squeezing.
...
In general, the polarization state of the light in the context of atom-light interaction, characterized by the Stokes vector $\mathbf{S} $, rotates on the \Poincare sphere under the following equation~\cite{Deutsch2010a}
\begin{align}
...
\end{align}
where OD is the total optical depth of the atoms\index{optical depth}, $ \Gamma $ is the spontaneous decay rate, and $ \Delta $ is the detuning. 

The optical depth for an atom coupled to an optical cavity can be defined as
\begin{align}
\mathrm{OD} &= \mathcal{F} \left( \frac{\sigma_0}{A}\right),
\end{align} 
where $ \mathcal{F} $ is the finesse of the cavity, $ \sigma_0 $ is the on-resonant cross section of the atom, and $ A $ is the effective mode area of the optical field at the atom position. 
Therefore, to 
\begin{align}
\frac{\mathrm{OD}}{N_A} &= \frac{\sigma_0}{A}.
\end{align}
The formula of OD per atom above has been commonly 
